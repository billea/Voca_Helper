            ...
            {drafts.map((d) => (
              <li key={d.id} className="p-3 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-slate-700">{new Date(d.createdAt).toLocaleString()}</span>
                  <span className="ml-auto text-slate-500">{d.wordCount} words • {Math.round(d.durationSec/60)} min</span>
                </div>
            ...
            {submissions.map((s) => (
              <li key={s.id} className="p-3 text-sm">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="ml-auto text-slate-500">{s.wordCount} words • {Math.round(s.durationSec/60)} min</span>
                </div>
                <div className="mt-1 line-clamp-2 text-slate-700">{s.content}</div>
                {s.nextStepTarget && (
                  <div className="mt-2">
                    <Link href={s.nextStepTarget} className="focus-ring inline-flex rounded-md bg-brand px-2 py-1 text-xs font-semibold text-white">Go to next step</Link>
                  </div>
                )}
              </li>
            ))}
